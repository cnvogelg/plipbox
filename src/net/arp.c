...
u08 arp_handle_packet(u08 *ethbuf, u16 ethlen, net_tx_packet_func tx_func)
{
...
const u08 *arp_find_mac(u08 *buf, const u08 *ip, net_tx_packet_func tx_func)
{
  /* is GW itself? */
  ...
  if(is_my_net) {
    ...
    if(cache_pos != ARP_CACHE_INVALID) {
      ...
      arp_send_request(buf, ip, tx_func);     
    }
  } else {
