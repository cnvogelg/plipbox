...
class Tap:
  ...
  def open(self):    
    if sys.platform == 'darwin':
      # Mac OS X needs TUN/TAP OSX and user must be member of group 'wheel'
      ...
      cmd = [self._sudo, self._tunctl, '-t', self.base_name, '-u', str(os.getuid())]
      self._run(cmd)
      # up interface
      ...
      fcntl.ioctl(self._fd, TUNSETIFF, struct.pack("16sH", self.base_name, IFF_TAP | IFF_NO_PI))
    else:
